...
## Lombok 이란?
**Lombok 은 자바에서 보일러플레이트 코드를 줄여주는 라이브러리입니다. 자바 개발 시 반복적으로 작성해야 하는 보일러플레이트 코드는 개발자의 생산성을 저하시키는 주요 요인 중 하나입니다. Lombok 은 이러한 문제를 해결하기 위해 등장한 라이브러리로, Annotation 을 사용하여 자동으로 코드를 생성해주는 기능을 제공합니다.    
예를 들어 `getter/setter`, `equals`, `hashCode`, `toString` 메서드와 같은 반복적인 코드를 자동으로 생성할 수 있어 코드가 더 깔끔하고 간결해집니다.    
