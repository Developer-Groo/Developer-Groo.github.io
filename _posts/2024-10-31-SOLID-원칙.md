...
## SOLID 원칙 이란?
**로버트 마틴이 2000년대 초에 명명한 객체 지향 프로그래밍의 다섯 가지 기본 원칙을 마이클 페더스가 원칙의 앞 글자를 따서 다시 SOLID라는 이름으로 소개한 것입니다. `SOLID` 원칙은 객체지향 설계에서 유지보수성과 확장성을 높이기 위한 다섯 가지 설계 원칙을 의미합니다. 이 원칙들은 시스템을 더 유연하고 확장 가능하게 만드는데 중요한 역할을 합니다.**
